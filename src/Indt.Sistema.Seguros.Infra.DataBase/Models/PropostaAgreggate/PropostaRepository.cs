...
namespace Indt.Sistema.Seguros.Infra.DataBase.Models.PropostaAgreggate
{
    internal class PropostaRepository : IPropostaRepository
    {
        ...
        public PropostaRepository(SegurosContext context, INotification notificacao)
        {
            _context = context;
            _notificacao = notificacao;
            _logger = Log.ForContext<PropostaRepository>();
        }
        ...
        public async ValueTask<(List<Proposta> Porpostas, int TotalProposta)> ListarAsync(FiltroPropostas filtroPropostas, CancellationToken cancellationToken = default)
        {
            ...
            var totalPorpostas = query.Count();
            ...
            return (propostas.Select(x=> MapearProposta(x)).ToList(), totalPorpostas);
        }
